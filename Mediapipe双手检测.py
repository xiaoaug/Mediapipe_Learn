...
# @File    : Mediapipe双手检测.py
# @Software: PyCharm
# @Desc    : 基于mediapipe的双手检测，检测到两只手及其坐标
...
# 主函数
def main():
    hand_detect = HandDetector(max_hands=2, detection_con=0.7)
    ...
    while True:
        ...
        if hand_info:  # 获取到手的位置信息
            if len(hand_info) == 2:  # 有两只手的情况下
                hand1_type = hand_info[0]["type"]  # 左手还是右手
                hand1_lms_list = hand_info[0]["lms_list"]  # 获取左手的21个关键点坐标
                # hand1_center = hand_info[0]['center']  # 手部中点坐标
                # finger1 = hand_detect.fingers_up(hand_info[0])

                hand2_type = hand_info[1]["type"]  # 左手还是右手
                hand2_lms_list = hand_info[1]["lms_list"]  # 获取左手的21个关键点坐标
                # hand2_center = hand_info[1]['center']  # 手部中点坐标
                # finger2 = hand_detect.fingers_up(hand_info[1])

                length, info = hand_detect.find_distance(
                    p1=hand2_lms_list[8], p2=hand1_lms_list[8], img=img, draw=True
                )
                cv2.putText(
                    img,
                    f"Hand: {hand1_type}, {hand2_type}",
                    (0, 55),
